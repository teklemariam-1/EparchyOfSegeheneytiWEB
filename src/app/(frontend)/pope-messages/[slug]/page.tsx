...
export default async function PopeMessageDetailPage({
  ...
  const [allMessages, mostViewed, vaticanNews, latestNews, adjacent] = await Promise.all([
    getPopeMessagesList(20, locale),
    getMostViewedPopeMessages(4, locale),
    getNewsList({ limit: 4, category: 'vatican', locale }).then((r) => r.docs),
    getNewsList({ limit: 3, locale }).then((r) => r.docs),
    getAdjacent('pope-messages', msg.publishedAt, locale),
  ])
  ...
  const sections: SidebarSection[] = [
    ...
    },
    {
      ...
      items: mostViewed
        ...
        .map((m) => ({ href: `/pope-messages/${m.slug}`, title: m.title, date: m.publishedAt, views: m.views })),
    },
    {
          ...
          <div className="grid gap-10 lg:grid-cols-4">
            <article className="lg:col-span-3">
              ...
              <header className="mb-6">
                <span className="mb-4 inline-block rounded-full border border-gold-200 bg-gold-50 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-gold-800">
                  {typeLabel}
                </span>
              ...
              {(adjacent.prev || adjacent.next) && (
                <nav aria-label="More messages" className="mt-10 grid gap-4 border-t border-charcoal-100 pt-6 sm:grid-cols-2 print:hidden">
                  {adjacent.prev ? (
                    <Link href={`/pope-messages/${adjacent.prev.slug}`} className="group rounded-xl border border-charcoal-100 p-4 transition-colors hover:border-maroon-300">
                  ...
                  {adjacent.next && (
                    <Link href={`/pope-messages/${adjacent.next.slug}`} className="group rounded-xl border border-charcoal-100 p-4 text-right transition-colors hover:border-maroon-300">
